...
$LogPath = Join-Path $LogDir "hourly.log"

New-Item -ItemType Directory -Path $LogDir -Force | Out-Null

if (-not (Test-Path -LiteralPath $CredentialPath) -or -not (Test-Path -LiteralPath $SettingsPath)) {
    "$(Get-Date -Format o) Telegram не настроен. Запустите setup-hourly-task.ps1." | Tee-Object -FilePath $LogPath -Append
    exit 2
}
...
try {
    "`n$(Get-Date -Format o) Начало ежечасного обновления" | Tee-Object -FilePath $LogPath -Append
